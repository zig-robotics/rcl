...
pub fn linkLibraryRecursive(src: *std.Build.Step.Compile, dependency: *std.Build.Step.Compile) void {
...
pub fn build(b: *std.Build) void {
    ...
    yaml_param_parser.installHeadersDirectory(upstream.path("rcl_yaml_param_parser/include"), "", .{});

    yaml_param_parser.addCSourceFiles(.{ .root = upstream.path("rcl_yaml_param_parser"), .files = &.{
        "src/add_to_arrays.c",
        "src/namespace.c",
        "src/node_params.c",
        "src/parse.c",
    ...
    rcl.addIncludePath(b.dependency("ros2_tracing", .{}).namedWriteFiles("tracetools").getDirectory());
    ...
    linkLibraryRecursive(rcl, yaml_param_parser);
